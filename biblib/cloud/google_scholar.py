...
def query_and_retrieve_metadata(q, only_first_record):
    """Search Google Scholar with a pre-formatted query like :
    intitle:"{0}"
    and extract the metadata in the specified format.
    if only_first_record == True only the first result in retrieve."""
    ...
    if only_first_record is True and len(metadata_links) != 0:

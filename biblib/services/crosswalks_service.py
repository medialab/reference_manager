...
#from biblib.crosswalks import html_crosswalk
from biblib.crosswalks import mets_crosswalk
from biblib.crosswalks import mods_crosswalk
from biblib.crosswalks import oai_dc_crosswalk
...
def convert_csv(input_data, input_format, source, rec_id_prefix, only_first_record):
    return csv_crosswalk.csv_dict_reader_to_metasjon_list(input_data, input_format, source, rec_id_prefix, only_first_record)


def convert_json(input_data, input_format, source, rec_id_prefix, only_first_record):
...
def convert_marc(input_data, input_format, source, rec_id_prefix, only_first_record):
    return unimarc_crosswalk.unimarc_marcreader_to_metasjon_list(input_data, source, rec_id_prefix, only_first_record)
...
def convert_txt_lines(txt_lines, input_format, source, rec_id_prefix, only_first_record):
    if txt_lines is not None:
        ...
        if input_format is not None:
            ...
            if input_format == constants.FORMAT_RIS:
                return ris_crosswalk.ris_txt_lines_to_metajson_list(txt_lines, source, rec_id_prefix, only_first_record)
            else:
                logging.error("Error: {} input_format not managed".format(input_format))
...
def parse_and_convert_csv(input_file_path, input_format, source, rec_id_prefix, only_first_record):
    try:
        csv_dict_reader = io_service.parse_csv(input_file_path)
        return convert_csv(csv_dict_reader, input_format, source, rec_id_prefix, only_first_record)
    finally:
        pass
...
def parse_and_convert_json_str(input_string, input_format, source, rec_id_prefix, only_first_record):
    json_data = io_service.parse_json_str(input_string)
    return convert_json(json_data, input_format, source, rec_id_prefix, only_first_record)


def parse_and_convert_marc(input_file_path, input_format, source, rec_id_prefix, only_first_record):
    try:
        marc_reader = io_service.parse_marc(input_file_path)
        return convert_marc(marc_reader, input_format, source, rec_id_prefix, only_first_record)
    finally:
        pass
...
def parse_and_convert_xmletree(input_file_path, input_format, source, rec_id_prefix, only_first_record):

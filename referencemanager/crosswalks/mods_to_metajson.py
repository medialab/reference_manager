...
def convert_mods_file_to_metajson_document_list(mods_filename):
    ...
    mods_parsed = ET.parse(mods_filename, parser=parser)
    mods_tree = mods_parsed.getroot()
    return convert_mods_tree_to_metajson_document(mods_tree, mods_filename)


def convert_mods_string_to_metajson_document(mods_string, source):
    register_namespaces()

    mods_tree = ET.fromstring(mods_string)
    return convert_mods_tree_to_metajson_document(mods_tree, source)


def convert_mods_tree_to_metajson_document(mods_tree, source):
    document_list = []

    if mods_tree is not None:
        if mods_tree.tag.endswith("mods"):
            document = convert_mods_to_metajson_document(mods_tree, source)
            document_list.append(document)
        elif mods_tree.tag.endswith("modsCollection"):
            mods_list = mods_tree.findall("mods")
            if mods_list:
                for mods in mods_list:
                    document_list.append(convert_mods_to_metajson_document(mods, source))

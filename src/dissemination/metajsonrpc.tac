...
from crosswalks import metajson_to_metajson_ui
...
class References_repository(jsonrpc.JSONRPC):
    ...
    def jsonrpc_save(self, document):
        ...
        return self.format_bson(mongodb_repository.save_reference(json_doc))
    ...
    def jsonrpc_save_ui(self, document_ui):
        ...
        return self.format_bson(mongodb_repository.save_reference(document))
    ...
    def jsonrpc_metadata_by_rec_ids(self, rec_ids, format="metajson"):
        ...
        """
        metajson_document = mongodb_repository.get_references_by_rec_ids(rec_ids)
        if format == "metajson":
    ...
    def jsonrpc_metadata_by_mongo_ids(self, mongo_ids, format="metajson"):
        """ get metadata of a list of references
            ...
            return the asked references in the specified format
        """
        metajson_document = mongodb_repository.get_references_by_mongo_ids(mongo_ids)
        if format == "metajson":
            return self.format_bson(metajson_document)
    ...
    def jsonrpc_citation_by_rec_ids(self, rec_ids, style="mla", format="html"):
        """ get citations of a list of references
            ...
            return the asked references in the specified format
        """
        document_list = mongodb_repository.get_references_by_rec_ids(rec_ids)
        if document_list:
            results = []
    ...
    def jsonrpc_search(self, query):
        ...
        """
        mongodb_repository.search_references(query)
    ...
    def jsonrpc_types(self, type_id, language):
        ...
        return self.format_bson(mongodb_repository.get_type(type_id))
    ...
    def jsonrpc_datafields(self, rec_type, language):
        ...
        return self.format_bson(mongodb_repository.get_datafield(rec_type))

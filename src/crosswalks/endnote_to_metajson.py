...
def extract_contributors(contributor_type, role, element,xpath):
    ...
    if authors_list is not None and len(authors_list) != 0 :
        ...
        for author_item in authors_list:
            contributors.extend(convert_endnote_authors_to_contributors(author_item,contributor_type,role))
        return contributors
...
def convert_endnote_authors_to_contributors(authors,contributor_type="person",role="aut"):
    contributors = []
    for author in authors.splitlines():
        contributor = metajson_contrib_util.convert_formatted_name_to_contributor(author,contributor_type,role)
        if contributor:
...
def convert_endnote_record_to_metajson_document(record, source):
    ...
    if is_part_of_type is not None :
        ...
        if is_part_of_is_part_of_type is not None :
            ...
            is_part_of.add_contributors(metajson_contrib_util.change_contibutors_role(secondary_contributors,"aut"))
            ...
            is_part_of_is_part_of.add_contributors(metajson_contrib_util.change_contibutors_role(translated_contributors,"edt"))
        ...
        if publisher:
            if endnote_type == TYPE_THESIS :
                document.add_contributors([metajson_contrib_util.convert_formatted_name_to_contributor(publisher,"orgunit","dgg")])
            elif endnote_type == TYPE_FILM_OR_BROADCAST :
                document.add_contributors([metajson_contrib_util.convert_formatted_name_to_contributor(publisher,"orgunit","dst")])
            else :
    ...
    # language
    if language :
        iso639_2 = language_util.convert_unknown_format_to_iso639_2(language)
        if iso639_2 :
    ...
    if debug:
        ...
        if is_part_of_is_part_of_type:related_items_msg+="\tis_part_of.is_part_of : {} ".format(is_part_of_is_part_of_type)
        print "# {}\t:\t{}\t:\t{}\t->\titem : {} {}".format(source,rec_id,endnote_type,rec_type,related_items_msg,title)

    return document


def convert_endnote_file_to_metajson_document_list(endnote_file):
    parser = ET.XMLParser(encoding = "utf-8")
    tree = ET.parse(endnote_file, parser = parser)
    records_elements = tree.getroot().find("records")
    records = records_elements.findall("record")

    document_list = []
    if records:
        for record in records:
            document_list.append(convert_endnote_record_to_metajson_document(record, endnote_file))

...
def test():
    ...
    input_dir = os.path.join(base_dir, "mods")
    metajson_list = []
    for file_name in os.listdir(input_dir):
        if file_name.endswith(".xml"):
            print file_name
            metajson_list.extend(mods_to_metajson.convert_mods_file_to_metajson_document_list(os.path.join(input_dir, file_name)))

    if metajson_list:
        output_path = os.path.join(base_dir, "result", "result_mods_metajon.json")
